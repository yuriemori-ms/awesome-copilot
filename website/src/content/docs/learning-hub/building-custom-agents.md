...
**include-custom-instructions** *(v1.0.86+, Copilot CLI)*: By default, a custom agent only follows the instructions in its own `.agent.md` file—repository instruction files like `AGENTS.md`, `copilot-instructions.md`, and `CLAUDE.md` are not automatically merged in. Set `include-custom-instructions: true` in the frontmatter to have the agent also pick up your repository's shared instruction files, so project-wide conventions apply on top of the agent's own persona and rules:
