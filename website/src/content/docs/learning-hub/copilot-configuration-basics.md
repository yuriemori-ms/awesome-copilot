...
**Auto routing tier defaults** *(v1.0.87+)*: Organizations and individual users can now set startup defaults for the Auto routing tier, controlling which tier Auto mode targets when a session starts. An organization can enforce a **strict** policy that locks the default for everyone, or leave it **user-overridable** so individual developers can adjust their own starting tier.
...
**`worktreePathTemplate` setting** *(v1.0.87+)*: Controls where `/worktree`, `/move`, `/new`, and `--worktree` create new worktrees. Set a template such as `~/src/worktrees/{repo}/{branch}` using the supported placeholders `{repoPath}`, `{repo}`, `{branch}`, and `{branchSlug}`. Leaving it unset keeps the existing `<repo>.worktrees/` layout, with slashes in branch names flattened to dashes.
