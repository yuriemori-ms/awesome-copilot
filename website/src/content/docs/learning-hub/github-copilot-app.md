...
*(v1.1.23+)* A new project setting and **`/sandbox`** command let you run an agent's shell commands in a local sandbox that restricts filesystem access to the session's workspace—giving you an extra guardrail against unintended reads or writes outside the project you're working in.

### Software Factories and My Work Filtering

*(v1.1.23+)* The Software Factories overview and factory list have been combined into a single page with a breadcrumb for viewing an individual factory. **My Work** is now split into separate **Issues** and **Pull Requests** sections in the sidebar and navigation, with repositories browsable as dedicated pages. You can also describe the results you want in My Work's **Add filter** menu and get a generated filter you can inspect, edit, or revert—handy when you don't remember the exact filter syntax.

### Restarting a Session

*(v1.1.23+)* Use the **`/restart-session`** command to restart a chat or side chat conversation while preserving its history, without needing to create a brand-new session.
