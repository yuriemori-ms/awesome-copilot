...
> **Machine-readable output (v1.0.85+)**: Add `--json` to `copilot plugin list`, `copilot plugin marketplace list`, and `copilot plugin marketplace browse` to get structured JSON output instead of the interactive table — useful for scripts that need to check installed plugins or available marketplace entries programmatically.
